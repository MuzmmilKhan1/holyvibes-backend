...
class ClassTimingsController extends Controller
{
    public function get_teacher_class_time($teacherID)
    {
        $class_time = ClassTimings::with(['course'])->where('teacherID', $teacherID)->get();
        if (!$class_time) {
    ...
    public function add_edit_time(Request $request)
    {
        ...
        if ($validatedData['classTimingID'] == 0) {
            ...
            $existingTime = ClassTimings::find($validatedData['classTimingID']);
            if (!$existingTime) {
                return response()->json([
    ...
    {
        $class_time = ClassTimings::find($classTimingID);
